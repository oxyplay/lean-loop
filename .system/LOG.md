> **AI Instruction (Pruning):** If this file exceeds 100 lines, compress only old closed `Deferred Issues` into one-line summaries. Keep `Technical Decisions` detailed if they remain relevant.

# Decision & Debt Log

## 🛡️ Technical Decisions
- **[ID-001]:** [Reasoning for choosing a specific library or architecture]

## ⚠️ Deferred Issues (Technical Debt)
- [ ] [Issue description to be addressed in a future milestone]

## 💥 Failure Log (Red Phase Post-Mortems)
- [Date/Task]: [Brief reason for test failure, identified during UNIFY]
